...
$preference = strtolower(trim($_GET['preference'] ?? ''));
...
if ($preference === '') {
  ...
  echo json_encode([
    'error' => 'Preference parameter is required'
  ]);
...
if (!isset($filterMap[$preference])) {
  ...
  echo json_encode([
    'error' => 'Invalid preference filter'
  ]);
...
try {
  $sql = "SELECT id, brand, name, price, image_path AS image, bodyType, fuelType, transmission, seats, isPopular, isLuxury, isUsed FROM cars WHERE {$filterMap[$preference]} ORDER BY id DESC";
